// ===============================
// Cargo.toml (put this in a separate file when you create the project)
// -------------------------------
// [package]
// name = "egui_rad_builder"
// version = "0.1.0"
// edition = "2021"
//
// [dependencies]
...
//!
//! Features:
//! - Sidebar palette of common controls.
//! - Drag from palette (or click-to-place) into a central design canvas.
//! - Drag to move; corner handle to resize.
//! - Property inspector for selected widget.
//! - "Generate Code" produces Rust/egui code that recreates the designed UI.
//!
//! NOTE: This is a compact prototype spanning a single file for clarity.
//! Split into modules as you evolve it.
...
impl RadBuilderApp {
    ...
    fn generate_code(&self) -> String {
        // We'll emit a self-contained snippet with a minimal eframe App and a GeneratedState.
        ...
        out.push_str(&format!("    let canvas = egui::Rect::from_min_size(ui.min_rect().min, egui::vec2({}, {}));\n", self.project.canvas_size.x, self.project.canvas_size.y));
        ...
        for w in &self.project.widgets {
            ...
            match w.kind {
                WidgetKind::Label => {
                    out.push_str(&format!(
                        "    ui.allocate_ui_at_rect(egui::Rect::from_min_size(ui.min_rect().min + egui::pos2({:.1},{:.1}), egui::vec2({:.1},{:.1})), |ui| {{ ui.label(\"{}\"); }});\n",
                        pos.x, pos.y, size.x, size.y, escape(&w.props.text)
                    ));
                }
                WidgetKind::Button => {
                    out.push_str(&format!(
                        "    ui.allocate_ui_at_rect(egui::Rect::from_min_size(ui.min_rect().min + egui::pos2({:.1},{:.1}), egui::vec2({:.1},{:.1})), |ui| {{ ui.add_sized(egui::vec2({:.1},{:.1}), egui::Button::new(\"{}\")); }});\n",
                        pos.x, pos.y, size.x, size.y, size.x, size.y, escape(&w.props.text)
                    ));
                }
                WidgetKind::Checkbox => {
                    out.push_str(&format!(
                        "    ui.allocate_ui_at_rect(egui::Rect::from_min_size(ui.min_rect().min + egui::pos2({:.1},{:.1}), egui::vec2({:.1},{:.1})), |ui| {{ ui.checkbox(&mut state.checked_{}, \"{}\"); }});\n",
                        pos.x, pos.y, size.x, size.y, w.id.0, escape(&w.props.text)
                    ));
                }
                WidgetKind::TextEdit => {
                    out.push_str(&format!(
                        "    ui.allocate_ui_at_rect(egui::Rect::from_min_size(ui.min_rect().min + egui::pos2({:.1},{:.1}), egui::vec2({:.1},{:.1})), |ui| {{ ui.add_sized(egui::vec2({:.1},{:.1}), egui::TextEdit::singleline(&mut state.text_{}).hint_text(\"{}\")); }});\n",
                        pos.x, pos.y, size.x, size.y, size.x, size.y, w.id.0, escape(&w.props.text)
                    ));
                }
                WidgetKind::Slider => {
                    out.push_str(&format!(
                        "    ui.allocate_ui_at_rect(egui::Rect::from_min_size(ui.min_rect().min + egui::pos2({:.1},{:.1}), egui::vec2({:.1},{:.1})), |ui| {{ ui.add_sized(egui::vec2({:.1},{:.1}), egui::Slider::new(&mut state.value_{}, {:.3}..={:.3}).text(\"{}\")); }});\n",
                        pos.x, pos.y, size.x, size.y, size.x, size.y, w.id.0, w.props.min, w.props.max, escape(&w.props.text)
                    ));
                }
                WidgetKind::ProgressBar => {
                    out.push_str(&format!(
                        "    ui.allocate_ui_at_rect(egui::Rect::from_min_size(ui.min_rect().min + egui::pos2({:.1},{:.1}), egui::vec2({:.1},{:.1})), |ui| {{ ui.add_sized(egui::vec2({:.1},{:.1}), egui::ProgressBar::new(state.progress_{}).show_percentage()); }});\n",
